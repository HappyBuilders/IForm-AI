...
本文档用于指导智能体读取详情页临时目录中的原始 JSON 文件。执行 AI 智能分析时，调用方只会在 prompt 中提供文件路径清单，不会把大体量 JSON 直接拼入请求参数。请先读取文件清单中的数据文件，再结合本文档理解字段含义和页签关联。

## 文件清单约定

临时文件位于 `.tmp/analysis/{sessionId}/`，常见文件如下：
